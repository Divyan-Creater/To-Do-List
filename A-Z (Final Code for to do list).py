...
class ToDoList:
    def __init__(self, root):
        ...
        self.mark_task_as_done_button = tk.Button(button_frame, text="Mark as Done", command=self.mark_task_done)
        self.mark_task_as_done_button.pack(side=tk.TOP, padx=5, pady=5)
    ...
    def delete_task(self):
        ...
        if selected_task_index:
            ...
            task = self.task_selection_box.get(index)
            self.tasks.pop(index) 
            self.task_selection_box.delete(index)  
            self.task_list_box.delete(index)  
    ...
    def mark_task_done(self):
        ...
        if selected_task_index:
            ...
            self.task_selection_box.delete(selected_task_index)
            self.task_selection_box.insert(selected_task_index, updated_task) 
            self.task_list_box.delete(selected_task_index)
            self.task_list_box.insert(selected_task_index, updated_task)  
